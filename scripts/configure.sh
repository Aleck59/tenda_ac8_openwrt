...
# configs/tenda_ac8.config sets CONFIG_ALL_KMODS: the kernel version hash
# of OpenWrt depends on the selected module packages, so the package feed
# can only offer modules built together with the images.  Drop the drivers
