...
v1 с флешем 8 МБ проверена загрузка: ядро, корневая ФС, коммутатор. Сеть и
Wi-Fi ещё отлаживаются. Флеш смонтирован только для чтения, настройки
...
**Сеть.** Ethernet на AC8 ещё калибруется. Подключите ПК к LAN-порту
(адрес `192.168.1.2/24`, непрерывный `ping 192.168.1.1`) и запустите в
консоли роутера `ac8-ethtest`. Скрипт подберёт задержки RGMII и режим
CPU-тега. Wi-Fi запускается только вручную (`rtl8197f-wlan-load --dual`),
LuCI его не видит. Подробности — в разделе 3 инструкции.
